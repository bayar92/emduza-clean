...
export default async function Page() {
  const intro = await prisma.mend.findFirst();

  let safeIntro = null;
  if (intro) {
    let img = intro.image ? intro.image.replace('public', '') : '';
    if (img && !img.startsWith('/')) img = '/' + img;
    safeIntro = { ...intro, image: img || null };
  }
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-blue-200 text-sm font-medium mt-1">
                ЭМДҮЗ-ийн даргын албан ёсны мэндчилгээ
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="container mx-auto px-6 py-10">
          <div className="flex gap-8 items-start">
            ...
            <main className="flex-1 min-w-0 bg-white rounded-2xl border border-gray-100 shadow-sm p-8 md:p-10">
              {safeIntro ? (
                <IntroClient intro={safeIntro} />
              ) : (
                <div className="py-16 text-center text-gray-400 text-sm">
                  Мэндчилгээний мэдээлэл байхгүй байна
                </div>
              )}
            </main>
          </div>
